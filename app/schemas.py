    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "full_name": "John Doe",
                "bio": "Data analyst who loves math."
            }
        }
...
class CalculationCreate(BaseModel):
    ...
    type: OperationType = Field(..., description="Operation type (Add, Subtract, Multiply, Divide, Power, Modulo)")

    @model_validator(mode='after')
    def validate_divisor(self):
        """Ensure divisor is not zero for division operations."""
        if self.type in {OperationType.DIVIDE, OperationType.MODULO} and self.b == 0:
...
class CalculationUpdate(BaseModel):
    ...
    def validate_divisor(self):
        """Ensure divisor is not zero for division operations."""
        if self.type and self.b == 0 and self.type in {OperationType.DIVIDE, OperationType.MODULO}:
            raise ValueError("Division by zero is not allowed")
        return self
...
class CalculationSummary(BaseModel):
    """Aggregated summary stats for user calculations."""
    total: int
