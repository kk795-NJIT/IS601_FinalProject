...
class TestOperationType:
    ...
    def test_operation_type_from_string(self):
        ...
        assert OperationType("Power") == OperationType.POWER
        assert OperationType("Modulo") == OperationType.MODULO
...
class TestCalculationCreateSchema:
    ...
    def test_valid_power(self):
        """Test creating calculation with valid power data."""
        calc = CalculationCreate(a=2.0, b=3.0, type=OperationType.POWER)
        assert calc.a == 2.0
        assert calc.b == 3.0
        assert calc.type == OperationType.POWER

    def test_valid_modulo(self):
        """Test creating calculation with valid modulo data."""
        calc = CalculationCreate(a=10.0, b=3.0, type=OperationType.MODULO)
        assert calc.a == 10.0
        assert calc.b == 3.0
        assert calc.type == OperationType.MODULO
    ...
    def test_modulo_by_zero_validation(self):
        """Test that modulo by zero raises validation error."""
        with pytest.raises(ValidationError) as exc_info:
            CalculationCreate(a=10.0, b=0.0, type=OperationType.MODULO)
        assert "Division by zero is not allowed" in str(exc_info.value)
    ...
    def test_division_by_zero_allowed_for_other_operations(self):
        ...
        calc_pow = CalculationCreate(a=10.0, b=0.0, type=OperationType.POWER)
        assert calc_pow.b == 0.0
    
    def test_without_user_id(self):
        """Test creating calculation without user_id (Module 14 - user_id comes from JWT)."""
...
class TestPowerOperation:
    ...
    def test_power_zero_exponent(self):
        op = PowerOperation()
        assert op.calculate(5.0, 0.0) == 1.0


class TestModuloOperation:
    """Test suite for ModuloOperation."""

    def test_modulo_positive(self):
        op = ModuloOperation()
        assert op.calculate(10.0, 3.0) == 1.0
...
class TestCalculationFactory:
    ...
    def test_create_power_operation(self):
        """Test factory creates PowerOperation."""
        op = CalculationFactory.create_operation("Power")
        assert isinstance(op, PowerOperation)

    def test_create_modulo_operation(self):
        """Test factory creates ModuloOperation."""
        op = CalculationFactory.create_operation("Modulo")
        assert isinstance(op, ModuloOperation)
    
    def test_unsupported_operation_raises_error(self):
        """Test that unsupported operation type raises ValueError."""
        with pytest.raises(ValueError) as exc_info:
            CalculationFactory.create_operation("SquareRoot")
        assert "Unsupported operation type" in str(exc_info.value)
        assert "SquareRoot" in str(exc_info.value)
    ...
    def test_calculate_convenience_method_power(self):
        """Test calculate convenience method for power."""
        result = CalculationFactory.calculate("Power", 2.0, 3.0)
        assert result == 8.0

    def test_calculate_convenience_method_modulo(self):
        """Test calculate convenience method for modulo."""
        result = CalculationFactory.calculate("Modulo", 10.0, 3.0)
        assert result == 1.0
...
class TestFactoryIntegration:
    ...
    def test_all_operations_with_same_inputs(self):
        ...
        power_result = CalculationFactory.calculate("Power", a, 2.0)
        assert power_result == 400.0

        modulo_result = CalculationFactory.calculate("Modulo", a, 3.0)
        assert modulo_result == 2.0
    ...
    def test_power_then_add(self):
        result = CalculationFactory.calculate("Power", 2.0, 3.0)
        result = CalculationFactory.calculate("Add", result, 5.0)
        assert result == 13.0
